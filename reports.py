...
class GDMBot(object):
	...
	def getModActions(self, id, days):
	...
	def getTopWAM(self, offset):
		if not self.startDate:
			dates = self.session.get('http://www.wikia.com/api/v1/WAM/MinMaxWamIndexDate');
	...
	def recordReports(self):
		for id, wiki in self.wikis.items():
			id = id.rstrip()
			reports = self.getReportedPosts(id)
			if reports:
	...
	def recordModActions(self):
		for id, wiki in self.wikis.items():
			...
			totalCount = 0
			actions = self.getModActions(id, 30)
			if actions and 'users' in actions:
				for user in actions['users']:
					...
					if user["userInfo"]["badgePermission"] is '': 
						continue
					modCount[user["userInfo"]["badgePermission"]] += int(user["totalCount"])
		...
		with open("data/reports.csv", "a") as dataFile:
			for id, wiki in self.wikis.items():
				...
				dataFile.write(str(id) + ',' + wiki['wiki'] + ',' 
					+ str(wiki['modCount']) + ',' + str(wiki['nonModCount']) + ',' 
